...
eco <- read_sf("data/spatial/ecoregions") %>% st_transform(4326) %>% 
    mutate( NOUMENON=mapkey$NOUMENON[match(NA_L2CODE,mapkey$NA_L2CODE)],
            ...
            ALPHA=mapkey$ALPHA[match(NA_L2CODE,mapkey$NA_L2CODE)],
            LABEL=case_when(
                !is.na(AUDIO) ~ sprintf(
                    "<b>%s</b><br/>
...
# political <- rnaturalearth::ne_states(c("canada", "united states of america", "mexico"),returnclass="sf") %>% st_transform(4326) %>%
#     filter(name_en != "Hawaii")
load("data/spatial/political.RData")
...
server <- function(input, output){
    ...
    showModal(modalDialog(
        ...
        HTML("Click a region to hear the song of its people."), 
    ...
    output$map <- renderLeaflet({
        leaflet(eco, options=leafletOptions(crs=epsg2163)) %>% 
            ...
            addPolygons(fillColor=~er_pal(NA_L2CODE), fillOpacity=1, color="#272727", weight=1, 
                        popup=~LABEL) %>% 
            addPolygons(data=political, fill=FALSE, color="#666666", weight=1, dashArray="5") %>%
    ...
    observeEvent( input$info, {
        showModal(modalDialog(
            ...
            HTML("From the minds of <a href=https://twitter.com/BenGFreeman1/status/1382011326784937984>Ethan Linck and Ben Freeman</a>, inspired by <a href=https://sora.unm.edu/sites/default/files/journals/condor/v039n01/p0009-p0010.pdf>Aldo Leopold's (1937)</a> description of \"the numenon of the Sierra Madre: the thick-billed parrot\".<br/><br/>"), 
            ...
            HTML("Audio: <a href=https://www.xeno-canto.org>xeno-canto</a>")
        ))
    })
